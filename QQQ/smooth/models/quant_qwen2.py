...
from QQQ.smooth.quantization.migration_qwen2 import migration
...
class QuantizedQwen2RMSNorm(nn.Module):
    def __init__(self, org_module):
        super(QuantizedQwen2RMSNorm, self).__init__()
...
class QuantizedQwen2Attention(QuantizedModule):
    def __init__(
...
class QuantizedQwen2DecoderLayer(QuantizedModule):
    def __init__(
        self, org_module, w_qconfig, a_qconfig, qinput=True, backend="academic"
    ):
        super().__init__(backend=backend)
        ...
        self.a_qconfig = a_qconfig
        self.qinput = qinput
        self.first_layer_norm = QuantizedQwen2RMSNorm(org_module.input_layernorm)
        ...
        self.second_layer_norm = QuantizedQwen2RMSNorm(
...
class QuantizedQwen2Model(QuantizedModule, ModuleUtilsMixin):
    def __init__(
        self, org_module, w_qconfig, a_qconfig, qinput=True, backend="academic"
    ):
        super().__init__(backend=backend)
...
class QuantizedQwen2ForCausalLM(PreTrainedModel, QuantizedModule):
    def __init__(
        self,
        org_module,
        w_qconfig,
